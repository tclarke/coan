...
class Builder:
    ...
    def update_version_number(self, scheme, new_version):
        ...
        try:
            ...
            version_number = version_info["RAWPHOTO_VERSION_NUMBER"]
        ...
        # Update RAWPhotoVersion.h
        version_info["RAWPHOTO_VERSION_NUMBER"] = '"' + version_number + '"'
        if scheme == "production":
            version_info["RAWPHOTO_IS_PRODUCTION_RELEASE"] = "true"
            if self.verbosity >= 1:
                print "Making a production release"
        else:
            version_info["RAWPHOTO_IS_PRODUCTION_RELEASE"] = "false"
            if self.verbosity >= 1:
    ...
    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into RAWPhoto workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "RAWPhoto.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating RAWPhoto.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating RAWPhoto.dep file"
...
def build_installer(aeb_platforms=[], aeb_output=None,
    ...
    manifest = dict()
    version_info = read_version_h()
    manifest["version"] = version_info["RAWPHOTO_VERSION_NUMBER"][1:-1]
    manifest["name"] = version_info["RAWPHOTO_NAME"][1:-1]
    manifest["description"] = version_info["RAWPHOTO_NAME_LONG"][1:-1]
    aebl_platform_str = ""
    ...
    out_path = os.path.abspath(join("Installer","RAWPhoto.aeb"))
