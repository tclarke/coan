...
class Builder:
    ...
    def update_version_number(self, scheme, new_version):
        ...
        try:
            ...
            version_number = version_info["NEF_VERSION_NUMBER"]
        ...
        # Update NefVersion.h
        version_info["NEF_VERSION_NUMBER"] = '"' + version_number + '"'
        if scheme == "production":
            version_info["NEF_IS_PRODUCTION_RELEASE"] = "true"
            if self.verbosity >= 1:
                print "Making a production release"
        else:
            version_info["NEF_IS_PRODUCTION_RELEASE"] = "false"
            if self.verbosity >= 1:
    ...
    def build_executable(self, clean_build_first, build_opticks, concurrency):
        ...
        if self.verbosity > 1:
            print "Done building NEF plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into NEF workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "NEF.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating NEF.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating NEF.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, target, clean, concurrency):
        solution_file = os.path.abspath("Code/NEF.sln")
...
def read_version_h():
    version_path = join("Code", "Include", "NefVersion.h")
