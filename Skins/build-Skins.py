...
class Builder:
    ...
    def build_executable(self, clean_build_first, build_opticks, concurrency):
        ...
        if self.verbosity > 1:
            print "Done building Skins plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into Skins workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "Skins.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating Skins.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating Skins.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, clean, concurrency):
        solution_file = os.path.abspath("Code/Skins.sln")
...
def read_version_h():
    version_path = join("Code", "Include", "SkinsVersion.h")
...
def build_installer(aeb_platforms=[], aeb_output=None, depend_path=None, verbosity=None):
    ...
    manifest[PF_AEBL + "version"] = [version_info["SKINS_VERSION_NUMBER"]]
    manifest[PF_AEBL + "name"] = [version_info["SKINS_NAME"]]
    manifest[PF_AEBL + "description"] = [version_info["SKINS_NAME_LONG"]]
    manifest[PF_AEBL + "targetPlatform"] = aeb_platforms

    out_path = os.path.abspath(join("Installer","Skins.aeb"))
    ...
    for plat in aeb_platforms:
        ...
        if plat_parts[0].startswith('win'):
            ...
            copy_file_to_zip(extension_plugin_path, target_plugin_path, "Skins.dll", zfile)
            ...
            extension_plugin_path = join(bin_dir, "PlugIns")
            target_plugin_path = join("platform", plat, "PlugIns")
            copy_file_to_zip(extension_plugin_path, target_plugin_path, "Skins.so", zfile)
        else:
