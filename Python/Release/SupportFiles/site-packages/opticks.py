...
def register_animation_callback(name=None, callback=None):
   """Connect to the frame changed signal on an animation controller.
      name = The name of the animation controller. If empty, get the
             active animation controller.
      callback = A generator object which will be resumed with
                 callback.send(frameTime) whenever the frame changes
                 in the animation controller. Anything yielded will
                 be ignored. The callback must be initialized with .next().
      The return value is a handle to the connection. When it is garbage
      collected, the connection will be detached so you need to keep a
      reference to the handle. 'del handle' will usually cause it to be
      garbage collected and detached.
      ...
      Example:
        def callback():
           ...
           while tm < 100.0:
              ...
              print "Got time %s" % tm
        cb = callback()
        cb.next() # initialize to the first yield
        handle = register_animation_callback("Animation 1", cb)
   """
   return _opticks.connect_animation(name, callback)
