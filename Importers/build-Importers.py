...
class Builder:
    ...
    def build_executable(self, clean_build_first, build_opticks, concurrency):
        ...
        if self.verbosity > 1:
            print "Building Importers plug-ins..."
        buildenv = os.environ
        ...
        if self.verbosity > 1:
            print "Done building Importers plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into Importers workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "Importers.dep")
        if not os.path.exists(extension_dep_file):
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating Importers.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, clean, concurrency):
        solution_file = os.path.abspath("Importers.sln")
