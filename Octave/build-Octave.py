...
class Builder:
    ...
    def build_executable(self, clean_build_first, build_opticks, concurrency):
        ...
        if self.verbosity > 1:
            print "Building Octave plug-ins..."
        buildenv = os.environ
        ...
        if self.verbosity > 1:
            print "Done building Octave plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into Octave workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "Octave.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating Octave.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating Octave.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, clean, concurrency):
        solution_file = os.path.abspath("Octave.sln")
