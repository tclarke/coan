...
class Builder:
    ...
    def build_executable(self, clean_build_first, build_opticks, concurrency):
        ...
        if self.verbosity > 1:
            print "Done building SAR plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into SAR workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "SAR.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating SAR.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating SAR.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, clean, concurrency):
        solution_file = os.path.abspath("Code/SAR.sln")
...
def build_installer(aeb_platforms=[], aeb_output=None, depend_path=None, sdk_version=None, verbosity=None):
    ...
    manifest[PF_AEBL + "version"] = [version_info["SAR_VERSION_NUMBER"]]
    manifest[PF_AEBL + "name"] = [version_info["SAR_NAME"]]
    manifest[PF_AEBL + "description"] = [version_info["SAR_NAME_LONG"]]
    ...
    out_path = os.path.abspath(join("Installer","SAR.aeb"))
    ...
    doc_path = os.path.abspath(join("Code", "Build", "DoxygenOutput"))
    copy_files_in_dir_to_zip(doc_path, join("content", "Help", "SAR"), zfile)
